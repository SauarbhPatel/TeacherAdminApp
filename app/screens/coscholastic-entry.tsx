/**
 * coscholastic-entry.tsx
 *
 * 3-step selector for Co-Scholastic Grade Report:
 *   Step 1 → Select Class  (GET /webservice/getClassSectionList)
 *   Step 2 → Select Section (from same response)
 *   Step 3 → Select Exam   (POST /Webservice/getCoscholasticExamByClass)
 * Then navigates to coscholastic-students screen.
 */
      ...
      {steps.map((s, i) => {
        ...
        return (
          <React.Fragment key={s.n}>
            <View style={bar.item}>
              <View
                style={[bar.circle, done && bar.done, active && bar.active]}
              >
                <Text style={[bar.num, (done || active) && bar.numActive]}>
                  {done ? "✓" : s.n}
                </Text>
              </View>
              <Text style={[bar.lbl, active && bar.lblActive]}>{s.label}</Text>
            </View>
            {i < 2 && <View style={[bar.line, done && bar.lineDone]} />}
          </React.Fragment>
        );
      })}
    </View>
...
// ─── Main ────────────────────────────────────────────────
export default function CoscholasticEntryScreen() {
  ...
  // Step 2 — sections from selected class
  const sections: ClassSection[] =
    classes.find((c) => c.class_id === sel.class_id)?.sections ?? [];

  // Step 3 — co-scholastic exams
  const [exams, setExams] = useState<CoscholasticExamItem[]>([]);
  ...
  // ── Load co-scholastic exams when step 3 opens ──────────
  const loadExams = useCallback(
    async (class_id: string, section_id: string) => {
      ...
      setExams([]);
      try {
        const res = await getCoscholasticExamList(
          {
            school_id: user.record.school_id,
            session_id: (user.record as any).session_id ?? null,
            class_id,
            section_id,
          },
          user.token,
        ...
        if (res.response_code === 200) {
          ...
          setExmError(res?.response_message || "Could not load exams.");
        }
      } catch (e: any) {
  ...
  // ── Select exam ─────────────────────────────────────────
  const pickExam = (exam: CoscholasticExamItem) => {
    router.push({
      pathname: "/screens/coscholastic-students" as any,
      params: {
    ...
    <View style={{ flex: 1, backgroundColor: Colors.surface }}>
      ...
      >
        ...
        <View style={styles.headerTop}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>Co-Scholastic</Text>
            <Text style={styles.sub}>Grade Report Entry</Text>
          </View>
        </View>
      ...
      >
        ...
        {step === 1 && (
          <>
            ...
            <Text style={styles.stepSub}>Choose the class for grade entry</Text>
                ...
                >
                  <View
                    style={[
                      styles.classIcon,
                      {
                        backgroundColor: ACCENT[i % ACCENT.length] + "18",
                      },
                    ]}
                  >
                    <Text
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.classSub}>
                      ...
                      {" · "}
                      {cls.sections.map((s) => s.section_name).join(", ")}
                    </Text>
                  </View>
              ...
              {sections.map((sec, i) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.secCard,
                    {
                      backgroundColor: ACCENT[i % ACCENT.length] + "13",
                      borderColor: ACCENT[i % ACCENT.length] + "40",
                    },
                  ]}
        ...
        {step === 3 && (
          <>
            ...
            <Text style={styles.stepSub}>
              Class {sel.class_name}-{sel.section_name} — choose an exam
            </Text>

            {exmLoading ? (
              <View style={styles.center}>
                <ActivityIndicator color={Colors.purple} size="large" />
                <Text style={styles.loadTxt}>Loading exams…</Text>
              </View>
            ) : exmError ? (
              <ErrorState
                msg={exmError}
                onRetry={() => loadExams(sel.class_id!, sel.section_id!)}
              />
            ) : exams.length === 0 ? (
              <View style={styles.center}>
                <Text style={{ fontSize: 36, marginBottom: 10 }}>📭</Text>
              ...
              exams.map((exam, i) => (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.examId}>Exam ID: {exam.exam_id}</Text>
                  </View>
                  <View
                    style={[
                      styles.examBadge,
