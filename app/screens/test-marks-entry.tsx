  ...
  const loadSubjects = useCallback(
    async (cid: string, sid: string) => {
      ...
      try {
        ...
        else setSubError(res.message || "Could not load subjects.");
      } catch (e: any) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.surface }}>
      ...
      >
        ...
        <View style={styles.headerTop}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.sub}>Class test / unit test scores</Text>
          </View>
        </View>
      ...
      >
          ...
          <>
            ...
            {clsLoading ? (
              <View style={styles.center}>
                <ActivityIndicator color={ACCENT_LIGHT} size="large" />
        ...
        {step === 3 && (
          <>
            <Text style={styles.stepTitle}>Select Subject & Date</Text>
            <Text style={styles.stepSub}>
            ...
            <View style={styles.dateCard}>
              <Text style={styles.dateLabel}>📅 Test / Exam Date *</Text>
            ...
            {/* Subject list */}
            <Text style={[styles.stepTitle, { fontSize: 14, marginTop: 4 }]}>
              Select Subject
            </Text>
            {subLoading ? (
              <View style={styles.center}>
                <ActivityIndicator color={ACCENT_LIGHT} size="large" />
                <Text style={styles.loadTxt}>Loading subjects…</Text>
              </View>
            ) : subError ? (
              <ErrorState
                msg={subError}
                onRetry={() => loadSubjects(selClassId, selSectionId)}
              />
            ) : subjects.length === 0 ? (
              <View style={styles.center}>
                <Text style={{ fontSize: 36, marginBottom: 10 }}>📭</Text>
                <Text style={{ color: Colors.text3 }}>No subjects found</Text>
              </View>
            ) : (
              subjects.map((sub, i) => {
                const isActive = selSubjectId === sub.subject_master_id;
                  ...
                  >
                    <View
                      style={[
                        styles.subjectIcon,
                        {
                          backgroundColor: isActive ? ac + "25" : ac + "18",
                        },
                      ]}
                    >
                      <Text style={[styles.subjectInitial, { color: ac }]}>
                        {sub.subject_name[0]}
                      </Text>
                    </View>
                    <Text
                      style={[styles.subjectName, isActive && { color: ac }]}
                    >
                      {sub.subject_name}
                    </Text>
                    {isActive && (
                      <View style={[styles.selBadge, { backgroundColor: ac }]}>
                        <Text style={styles.selBadgeTxt}>✓ Selected</Text>
                      </View>
                    )}
                    {!isActive && <Text style={styles.chevron}>›</Text>}
                  </TouchableOpacity>
                );
              })
            )}
