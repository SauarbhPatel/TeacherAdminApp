/**
 * exam-attendance-students.tsx
 *
 * Per-student exam attendance editor.
 * Fields: classes_held (numeric), classes_attended (numeric), remarks (text).
 * Auto-saves on input blur. Optimistic UI. SaveToast.
 * Search + Sort (name A-Z, Z-A, adm no, roll no) — same pattern as marks-students.tsx
 */
...
import { getExamAttendance, saveExamAttendance } from "@/services/api";
import { ExamAttendanceStudent } from "@/types";
import { Colors, Spacing, Radius, Shadow } from "@/constants/theme";

// ─── Sort options (+ Roll No as requested) ───────────────
...
function buildFullName(s: ExamAttendanceStudent): string {
    return (
...
// ─── Local student type ───────────────────────────────────
interface LocalStudent extends ExamAttendanceStudent {
    localHeld: string;
    localAttended: string;
    localRemarks: string;
    saving: boolean;
    saved: boolean;
    dirty: boolean;
...
function SaveToast({ visible }: { visible: boolean }) {
    ...
    return (
        <Animated.View style={[toastS.wrap, { opacity }]} pointerEvents="none">
            <Text style={toastS.txt}>✓ Attendance saved</Text>
        </Animated.View>
    );
}
...
function StudentCard({
    ...
    onChange: (
        id: string,
        field: "localHeld" | "localAttended" | "localRemarks",
        val: string,
    ) => void;
    ...
    const held = parseInt(student.localHeld) || 0;
    const attended = parseInt(student.localAttended) || 0;
    const pct = held > 0 ? ((attended / held) * 100).toFixed(1) : "0.0";
    const pctNum = parseFloat(pct);
    const pctColor =
        pctNum >= 75 ? Colors.green : pctNum >= 50 ? Colors.amber : Colors.red;
    const hasError = attended > held && held > 0;
    ...
    return (
        ...
        >
            ...
            <View style={sc.topRow}>
                ...
                {/* Percentage badge */}
                <View
                    style={[sc.pctBadge, { backgroundColor: pctColor + "18" }]}
                >
                    <Text style={[sc.pctNum, { color: pctColor }]}>{pct}%</Text>
                    <Text style={[sc.pctLbl, { color: pctColor }]}>
                        attendance
                    </Text>
                </View>
            </View>
            ...
            {/* Fields row */}
            <View style={sc.fieldsRow}>
                {/* Classes Held */}
                <View style={sc.fieldBox}>
                    <Text style={sc.fieldLbl}>Classes Held</Text>
                    <TextInput
                        style={[sc.fieldInput, hasError && sc.fieldInputErr]}
                        value={student.localHeld}
                        onChangeText={(v) =>
                            onChange(
                                student.student_id,
                                "localHeld",
                ...
                {/* Classes Attended */}
                <View style={sc.fieldBox}>
                    <Text style={sc.fieldLbl}>Attended</Text>
                    <TextInput
                        style={[sc.fieldInput, hasError && sc.fieldInputErr]}
                        value={student.localAttended}
                        onChangeText={(v) =>
                            onChange(
                                student.student_id,
                                "localAttended",
                ...
                {/* Weightage (read-only from API) */}
                <View style={sc.fieldBox}>
                    <Text style={sc.fieldLbl}>Weightage</Text>
                    <View style={[sc.fieldInput, sc.fieldReadonly]}>
                        <Text style={sc.fieldReadonlyTxt}>
                            {student.attendance.weightage ?? 0}
                        </Text>
                    </View>
                </View>
            </View>
            ...
            {hasError && (
                <Text style={sc.errMsg}>
                    ⚠ Attended cannot exceed classes held
                </Text>
            )}

            {/* Remarks */}
            <View style={{ marginTop: 10 }}>
                <Text style={sc.fieldLbl}>Remarks</Text>
                <TextInput
                    style={sc.remarksInput}
                    value={student.localRemarks}
                    onChangeText={(v) =>
                        onChange(student.student_id, "localRemarks", v)
                    }
                    onBlur={() => {
                        if (!hasError) onSave(student);
                    }}
            ...
            {student.dirty && !student.saving && (
                <Text style={sc.dirtyLbl}>● unsaved changes</Text>
            )}
        </View>
    );
}
...
const sc = StyleSheet.create({
    ...
    cardSaved: { borderColor: Colors.blueBg },
    topRow: {
        flexDirection: "row",
        alignItems: "center",
        gap: 10,
    ...
    pctNum: { fontSize: 16, fontWeight: "900" },
    pctLbl: { fontSize: 8, fontWeight: "600", marginTop: 1 },
    barWrap: {
        height: 4,
    ...
    fieldInputErr: { borderColor: Colors.red, backgroundColor: Colors.redBg },
    fieldReadonly: {
        justifyContent: "center",
        alignItems: "center",
        backgroundColor: Colors.surface + "88",
    },
    fieldReadonlyTxt: { fontSize: 15, fontWeight: "800", color: Colors.text2 },
    remarksInput: {
        backgroundColor: Colors.surface,
        borderRadius: Radius.md,
        borderWidth: 1.5,
...
// MAIN SCREEN
// ═══════════════════════════════════════════════════════
export default function ExamAttendanceStudentsScreen() {
    ...
    const fetchData = useCallback(
        async (silent = false) => {
            ...
            try {
                const res = await getExamAttendance(
                    {
                ...
                if (res.response_code === 200) {
                    const mapped: LocalStudent[] = (res.students ?? []).map(
                        (s) => ({
                            ...s,
                            localHeld: String(s.attendance.classes_held ?? ""),
                            localAttended: String(
                                s.attendance.classes_attended ?? "",
                            ),
                            localRemarks: String(s.attendance.remarks ?? ""),
                            saving: false,
                            saved: true,
                            dirty: false,
                        }),
    ...
    const handleChange = useCallback(
        (
            ...
            setStudents((prev) =>
                prev.map((s) =>
                    s.student_id === id
                        ? { ...s, [field]: val, dirty: true, saved: false }
                        : s,
                ),
            );
        },
    ...
    const handleSave = useCallback(
        async (student: LocalStudent) => {
            ...
            const held = parseInt(student.localHeld) || 0;
            const attended = parseInt(student.localAttended) || 0;
            if (attended > held && held > 0) return; // has error, don't save
            setStudents((prev) =>
                prev.map((s) =>
                    ...
                    <View style={styles.headerTop}>
                        ...
                        <View style={{ flex: 1 }}>
                            ...
                            <Text style={styles.headerSub}>
                                {loading
                                    ? "Loading…"
                                    : `${summary.total} students · Exam Attendance`}
                            </Text>
                        </View>
                        <TouchableOpacity
