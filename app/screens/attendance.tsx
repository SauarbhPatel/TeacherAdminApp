...
// ─── Mini bar ─────────────────────────────────────────────
function MiniBar({ present, absent, leave, total }: {
    ...
    return (
        <View style={bar.wrap}>
            {p > 0 && <View style={[bar.seg, { backgroundColor: Colors.green, width: `${p}%` }]} />}
            {l > 0 && <View style={[bar.seg, { backgroundColor: Colors.amber, width: `${l}%` }]} />}
            {a > 0 && <View style={[bar.seg, { backgroundColor: Colors.red, width: `${a}%` }]} />}
        </View>
    );
}
const bar = StyleSheet.create({
    wrap: { height: 5, flexDirection: "row", borderRadius: 3, overflow: "hidden", backgroundColor: Colors.border, marginTop: 10 },
    seg: { height: 5 },
});
...
function StatusBadge({ marked }: { marked: boolean }) {
    return (
        <View style={[badge.wrap, { backgroundColor: marked ? Colors.greenBg : Colors.amberBg }]}>
            <View style={[badge.dot, { backgroundColor: marked ? Colors.green : Colors.amber }]} />
            <Text style={[badge.text, { color: marked ? Colors.greenText : Colors.amberText }]}>
                {marked ? "Marked" : "Pending"}
            </Text>
        </View>
    );
...
export default function AttendanceScreen() {
        ...
        async (date: string, silent = false) => {
            ...
            try {
                ...
                if (res.response_code === 200) {
                    ...
                    setError(res.response_message || "Failed to load attendance.");
                }
            } catch (e: any) {
                setError(e?.message || "Network error. Please try again.");
            } finally {
    ...
    const classes = useMemo(() => {
        ...
        return allClasses.filter(c => !c.mark_by || c.mark_by.trim() === "" || c.mark_by === "-");
    }, [allClasses, filterPending]);

    // ── Summary values ───────────────────────────────────────
    const totalStudents = total?.total_students ?? allClasses.reduce((s, c) => s + parseNum(c.total_students), 0);
    const totalPresent  = total?.total_present  ?? allClasses.reduce((s, c) => s + parseNum(c.total_present), 0);
    const totalAbsent   = total?.total_absent   ?? allClasses.reduce((s, c) => s + parseNum(c.total_absent), 0);
    const totalLeave    = total?.total_leave    ?? allClasses.reduce((s, c) => s + parseNum(c.total_late), 0);
    const markedCount   = allClasses.filter(c => c.mark_by && c.mark_by.trim() !== "" && c.mark_by !== "-").length;
        ...
        <View style={{ flex: 1, backgroundColor: Colors.surface }}>
            ...
            >
                ...
                <View style={styles.headerTop}>
                    ...
                    <TouchableOpacity style={styles.backBtn} onPress={() => fetchData(selectedDate, true)} activeOpacity={0.75}>
                ...
                >
                    {weekDates.map(d => (
                        <TouchableOpacity
                            ...
                            activeOpacity={0.75}
                        >
                            <Text style={[styles.dcDay, selectedDate === d.iso && styles.dcActive]}>
                                {d.day}
                            </Text>
                            <Text style={[styles.dcNum, selectedDate === d.iso && styles.dcActive]}>
                                {d.num}
                            </Text>
                        </TouchableOpacity>
                    ))}
                ...
                {!loading && (
                    <View style={styles.summaryRow}>
                        {[
                            { num: totalStudents, label: "Students",   color: "#c4b5fd" },
                            { num: totalPresent,  label: "Present",    color: "#4ade80" },
                            { num: totalAbsent,   label: "Absent",     color: "#f87171" },
                            { num: totalLeave,    label: "Leave",      color: "#fbbf24" },
                        ].map((s, i) => (
            ...
            <View style={styles.statusBar}>
                ...
                <Text style={styles.statusTotal}>{allClasses.length} classes total</Text>
            </View>
            ...
            {filterPending && !loading && (
                <View style={styles.filterBanner}>
                    <Text style={styles.filterBannerTxt}>
                        ⏳ Showing {classes.length} pending class{classes.length !== 1 ? "es" : ""} only
                    </Text>
                    <TouchableOpacity
                ...
                >
                    {classes.length === 0 ? (
                        <View style={styles.emptyWrap}>
                            ...
                            <Text style={styles.emptyTitle}>
                                {filterPending ? "All classes marked!" : "No classes found"}
                            </Text>
                            <Text style={styles.emptyMsg}>
                                {filterPending
                        ...
                        classes.map((cls, i) => {
                            const isMarked = !!(cls.mark_by && cls.mark_by.trim() !== "" && cls.mark_by !== "-");
                            const total_s = parseNum(cls.total_students);
                            const p = parseNum(cls.total_present);
                            const a = parseNum(cls.total_absent);
                            const l = parseNum(cls.total_late);
                            ...
                            return (
                                <TouchableOpacity
                                    key={`${cls.class_id}-${cls.section_id}-${i}`}
                                    style={[styles.classCard, isMarked && styles.classCardMarked]}
                                    onPress={() =>
                                        router.push({
                                            pathname: "/screens/attendance-mark",
                                            params: {
                                                class_id: cls.class_id,
                                                section_id: cls.section_id,
                                                class_name: cls.class_name,
                                ...
                                >
                                    {/* Card top row */}
                                    <View style={styles.cardTop}>
                                        <View style={[styles.classIconWrap, { backgroundColor: isMarked ? Colors.greenBg : Colors.purpleBg }]}>
                                            <Text style={styles.classIconText}>
                                                {isMarked ? "✅" : "📋"}
                                            </Text>
                                        </View>
                                        <View style={{ flex: 1 }}>
                                            ...
                                            <Text style={styles.classStudents}>
                                                {total_s} students
                                                {cls.mark_by && cls.mark_by.trim() && cls.mark_by !== "-"
                                                    ? ` · Marked by ${cls.mark_by}`
                                                    : ""}
                                            </Text>
                                        </View>
                                    ...
                                    {/* Mini bar */}
                                    <MiniBar present={p} absent={a} leave={l} total={total_s} />

                                    {/* Stat boxes */}
                                    <View style={styles.statsRow}>
                                        {[
                                            { val: p, pct: pct(p, total_s), label: "Present", bg: Colors.greenBg, color: Colors.greenText },
                                            { val: a, pct: pct(a, total_s), label: "Absent",  bg: Colors.redBg,   color: Colors.redText   },
                                            { val: l, pct: pct(l, total_s), label: "Leave",   bg: Colors.amberBg, color: Colors.amberText  },
                                            { val: h, pct: pct(h, total_s), label: "Half",    bg: Colors.blueBg,  color: Colors.blueText   },
                                        ].map((s, j) => (
                                            <View key={j} style={[styles.statBox, { backgroundColor: s.bg }]}>
                                                <Text style={[styles.statVal, { color: s.color }]}>{s.val}</Text>
                                                <Text style={[styles.statPct, { color: s.color }]}>{s.pct}%</Text>
                                    ...
                                    {!isMarked && (
                                        <View style={styles.pendingHint}>
                                            <Text style={styles.pendingHintText}>Tap to mark attendance →</Text>
                                        </View>
                                    )}
                                </TouchableOpacity>
                            );
...
const styles = StyleSheet.create({
    // Header
    header: { paddingHorizontal: Spacing.xl, paddingBottom: 0, position: "relative", overflow: "hidden" },
    ...
    summaryPill: { flex: 1, borderRadius: 12, paddingVertical: 10, alignItems: "center", backgroundColor: "rgba(255,255,255,0.14)" },
    summaryNum: { fontSize: 18, fontWeight: "900" },
    summaryLabel: { fontSize: 9, fontWeight: "600", color: "rgba(255,255,255,0.7)", marginTop: 2 },
    ...
    // Filter banner
    filterBanner: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: Colors.amberBg, paddingHorizontal: Spacing.lg, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: Colors.amber + "40" },
    filterBannerTxt: { fontSize: 13, fontWeight: "700", color: Colors.amberText },
    filterClear: { fontSize: 12, fontWeight: "700", color: Colors.amber },

    // Class cards
    classCard: { backgroundColor: Colors.card, borderRadius: Radius.xl, padding: Spacing.lg, marginBottom: 10, ...Shadow.sm, borderWidth: 1.5, borderColor: Colors.border },
    classCardMarked: { borderColor: Colors.greenBg },
    cardTop: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 4 },
    classIconWrap: { width: 44, height: 44, borderRadius: 14, alignItems: "center", justifyContent: "center" },
    classIconText: { fontSize: 20 },
    className: { fontSize: 15, fontWeight: "800", color: Colors.text1 },
    classStudents: { fontSize: 12, color: Colors.text3, marginTop: 2 },
    ...
    // Stats inside card
    statsRow: { flexDirection: "row", gap: 6, marginTop: 12 },
    statBox: { flex: 1, borderRadius: 10, paddingVertical: 8, alignItems: "center" },
    statVal: { fontSize: 16, fontWeight: "900" },
    statPct: { fontSize: 9, fontWeight: "700", opacity: 0.75 },
    statLbl: { fontSize: 9, fontWeight: "600", marginTop: 1 },

    pendingHint: { backgroundColor: Colors.purplePale, borderRadius: 10, padding: 10, marginTop: 8, alignItems: "center" },
    pendingHintText: { fontSize: 12, fontWeight: "600", color: Colors.purple },
    ...
    loadingText: { marginTop: 12, fontSize: 14, color: Colors.text3 },
    errorEmoji: { fontSize: 40, marginBottom: 12 },
    errorTitle: { fontSize: 16, fontWeight: "700", color: Colors.text1, marginBottom: 6 },
    errorMsg: { fontSize: 13, color: Colors.text3, textAlign: "center", lineHeight: 20 },
    retryBtn: { marginTop: 16, backgroundColor: Colors.purple, borderRadius: 12, paddingHorizontal: 24, paddingVertical: 12 },
    retryText: { color: "#fff", fontWeight: "700", fontSize: 14 },
    emptyWrap: { alignItems: "center", paddingTop: 60 },
    emptyEmoji: { fontSize: 44, marginBottom: 12 },
    emptyTitle: { fontSize: 16, fontWeight: "700", color: Colors.text1 },
    emptyMsg: { fontSize: 13, color: Colors.text3, marginTop: 4, textAlign: "center" },
});