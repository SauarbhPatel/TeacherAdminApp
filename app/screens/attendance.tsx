...
function MiniBar({
  ...
  return (
    <View style={bar.wrap}>
      {p > 0 && (
        <View
          style={[bar.seg, { backgroundColor: Colors.green, width: `${p}%` }]}
        />
      )}
      {l > 0 && (
        <View
          style={[bar.seg, { backgroundColor: Colors.amber, width: `${l}%` }]}
        />
      )}
      {a > 0 && (
        <View
          style={[bar.seg, { backgroundColor: Colors.red, width: `${a}%` }]}
        />
      )}
    </View>
  );
...
export default function AttendanceScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      fetchData(selectedDate,allClasses?.length>0?true:false);
    }, [selectedDate]),
  );
                  ...
                  <View style={styles.statsRow}>
                    {[
                      ...
                      >
                        ...
                        <Text style={[styles.statPct, { color: s.color }]}>
                          {s.pct}%
                        </Text>
                        <Text style={[styles.statLbl, { color: s.color }]}>
                          {s.label}
                        </Text>
                      </View>
                    ))}
